...
class PPO():
    def __init__(self):
        ...
        self.Buffer = []
        self.counter = 0
        self.entropy_rate = 0.0005
        self.update_times = 10 #每一次update方法里循环更新的次数
        self.count_update = 0 #update的次数
    ...
    def update(self):
        ...
        rewards = torch.Tensor([t.reward for t in self.Buffer]).to(device).view(-1, 1)
        ...
        with torch.no_grad():
            target_v = rewards + GAMMA * self.critic(next_states)
        ...
        for i in range(self.update_times):
            for index in BatchSampler(SubsetRandomSampler(range(len(self.Buffer))), BATCH_SIZE, False):
                ...
                actor_loss = -torch.min(surr1, surr2).mean() - self.entropy_rate*torch.mean(entropy)
...
def main():
    ...
    for i_episode in range(episodes):
        ...
        if i_episode % 10 == 0:
            print("episode: {}, entropy_rate:{}, running reward is: {}, episode_rewards: {}".\
                  format(i_episode, PPO_Agent.entropy_rate, round(running_reward, 3), round(ep_rewards)))
